package definitivoSietePuntoDosUno;
...
public class ConexionEmpleado {
	...
	public List<Empleado> getEmpleados() {
		...
		try {
			...
			while (rs.next()) {
				Empleado emp = new Empleado(rs.getString("dni"), rs.getString("nombre"), rs.getString("apellido1"),
	 ...
	 * 
	 * 
	 * public void buscarEmpleadoPorDni(String dni) { try {
	 * System.out.println("busquemos empleado por dni"); ResultSet rs;
	 * PreparedStatement consulta =
	 * con.prepareStatement("select * from empleados where dni=?");
	 * consulta.setString(1, dni); rs = consulta.executeQuery(); while (rs.next()) {
	 * System.out.println("las coincidencias son :");
	 * System.out.println(rs.getString("dni") + " " + rs.getString("nombre") + " " +
	 * rs.getString("apellido1") + "  " + rs.getString("apellido2") + "  " +
	 * rs.getFloat("sueldo")); } } catch (Exception e) { } }
	 */

	/*
	 * 
	 * 
	 * public ResultSet consultarTodo() { String consulta=" select * from emleados";
	 * ResultSet rs; try { PreparedStatement sentencia =
	 * con.prepareStatement(consulta); rs = sentencia.executeQuery(); while
	 * (rs.next()) { System.out.println(rs.getString("dni") + " " +
	 * rs.getString("nombre") + " " + rs.getString("apellido1") + "  " +
	 * rs.getString("apellido2") + "  " + rs.getFloat("sueldo")); } } catch
	 * (SQLException e) { e.printStackTrace(); return null; } return rs; }
	 * 
	 */
